...
// ============ Camera Scanning Lifecycle (PWA) ============

let activeStream = null;

function startCameraScan() {
    const screenGame = document.getElementById('screen-game');
    const screenCamera = document.getElementById('screen-camera-scan');
    const video = document.getElementById('webcam-preview');

    screenGame.classList.remove('active');
    screenCamera.classList.add('active');

    // Request device camera (facingMode environment requests back camera on phones)
...
function captureBoardFrame() {
    // When capturing in a real app, this grabs a canvas frame and parses it.
    // In our PWA simulator, we simulate a scan by randomizing the board 
    // to mock active coin layout detection, then trigger auto-analysis.
    closeCameraScan();
    randomizeBoard();
    analyzeBoard();
}
